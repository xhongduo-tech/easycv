...
export function NewResumeDialog({
  ...
  async function createResume() {
    ...
    const recommendedTemplate = target
      ? recommendedTemplateIdsFor(target).find((id) => templates.some((item) => item.id === id))
      : undefined;
    const templateId = recommendedTemplate ?? templates[0]?.id;
    ...
    try {
      ...
      const response = await fetch("/api/resumes", {
        ...
        body: JSON.stringify({
          ...
          templateId,
          title: `${targetName} · ${track === "study" ? "申请 CV" : "求职简历"}`,
  ...
  return (
    <div className={styles.dialogBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={dialogRef} className={styles.newResumeDialog} role="dialog" aria-modal="true" aria-labelledby="new-resume-title">
        <div className={styles.dialogHeader}>
          <div><span><Sparkles size={17} /></span><div><small>新建简历</small><h2 id="new-resume-title">先告诉我们你的目标</h2></div></div>
          <button ref={closeRef} type="button" onClick={onClose} aria-label="关闭新建简历"><X size={20} /></button>
        </div>
        ...
        <div className={styles.dialogBody}>
          <section className={styles.createSection}>
            ...
            <div className={styles.purposeChoices}>
              <button type="button" aria-pressed={track === "study"} className={track === "study" ? styles.purposeActive : ""} onClick={() => changeTrack("study")}>
                <GraduationCap size={21} /><div><strong>留学申请</strong><small>院校 · 项目 · 学位</small></div>{track === "study" && <Check size={17} />}
              </button>
              <button type="button" aria-pressed={track === "career"} className={track === "career" ? styles.purposeActive : ""} onClick={() => changeTrack("career")}>
          ...
          {track && (
            <section className={styles.createSection}>
              ...
              <div className={styles.groupChips} role="group" aria-label={track === "study" ? "院校地区" : "企业类型"}>
                {groups.map((group) => <button type="button" key={group.key} aria-pressed={!query && selectedGroup === group.key} className={!query && selectedGroup === group.key ? styles.groupActive : ""} onClick={() => { setSelectedGroup(group.key); setQuery(""); setSelectedTarget(""); setCustomTarget(""); }}>{group.label}<small>{group.count}</small></button>)}
              </div>
              <label className={styles.targetSearch}><Search size={17} /><span className="sr-only">搜索目标</span><input value={query} onChange={(event) => { setQuery(event.target.value); setSelectedTarget(""); }} placeholder={`搜索${track === "study" ? "院校或地区" : "企业或行业"}`} /></label>
              {loading ? <div className={styles.targetLoading}><LoaderCircle size={19} /> 正在加载目标…</div> : (
                <div className={styles.targetOptions}>
                  {visibleTargets.map((item) => <button type="button" key={item.id} aria-pressed={selectedTarget === item.id} className={selectedTarget === item.id ? styles.targetActive : ""} onClick={() => { setSelectedTarget(item.id); setCustomTarget(""); setError(""); }}><span>{track === "study" ? <GraduationCap size={16} /> : <Building2 size={16} />}</span><div><strong>{item.name}</strong><small>{item.region} · {item.category}</small></div>{selectedTarget === item.id && <Check size={15} />}</button>)}
                </div>
              )}
              <label className={styles.customTarget}><span>没有找到？</span><input value={customTarget} onChange={(event) => { setCustomTarget(event.target.value); if (event.target.value) setSelectedTarget(""); }} placeholder={`输入自定义${track === "study" ? "院校或项目" : "企业或岗位"}`} /></label>
            </section>
          )}
        ...
        <div className={styles.dialogFooter}>
          ...
          <button className="button button-primary" type="button" disabled={creating || loading || !track || (!target && !customTarget.trim())} onClick={() => void createResume()}>
