...
Advisor 在平台 API 内提供两层能力：默认确定性规则分析，以及由服务端环境变量启用的 OpenAI Responses API 适配层。外部模型只有在用户对本次请求显式同意后才会调用：
...
- 模型不能自行决定证据来源：原文定位必须匹配服务端提供的候选，岗位要求与引用证据由确定性证据地图重新绑定。新增数字、常见工具词、伪造定位、占位符或缺失事实未清空的草稿不能直接应用。
...
- 同一模型连续三次网络、超时、429 或 5xx 错误后开启 10 分钟熔断；内容拒答、普通 4xx 与输出校验失败只回退当前请求，不允许用户内容触发全局停用。
