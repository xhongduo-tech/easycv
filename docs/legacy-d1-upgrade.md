...
本文只适用于线上库由旧版请求期初始化器创建、业务数据仍在，并且经只读检查
确认**平台迁移台账确实不存在**的情况。Sites 使用
`__appgarden_migrations` 记录标准迁移；如果它已存在且内容严格为
`0000`–`0004`，不需要也不得运行 bridge，应直接由 Sites 的标准迁移阶段执行
`0005`–`0011`。不要用本流程修复未知、半成品结构或已有但异常的平台台账。
...
- 一次采纳请求只允许一次完整结构检查；创建/补记前后的复读只能检查
  `d1_migrations` 本身。正常可采纳路径的 D1 statement 总数必须严格少于 50。
...
先通过 Sites 的只读数据库检查能力确认是否存在
`__appgarden_migrations`，并按 `id` 读取其 `name` 与 `applied_at`。不得为了检查
而创建、删除、改名或写入任何台账。

- 表存在，且按 `id` 排序后恰好是 `0000_silly_thanos.sql` 至
  `0004_complete_auth.sql`：跳过本文第 1、2 节，直接进入第 3 节。Sites 会把
  这五项视为已完成，只执行后续标准迁移。
- 表存在，但结构异常、名称不是严格前缀、包含未知名称或状态无法确认：停止。
  不要用 `d1_migrations` 覆盖、旁路或“修复”平台台账。
- 表确实不存在：才允许继续 bridge 流程，并仍需通过严格旧结构指纹检查。

`__appgarden_migrations` 是 Sites 的平台台账；本脚本中的 `d1_migrations` 只是
缺失平台台账这一特殊旧部署场景的兼容基线，二者不能相互猜测或手工复制。
...
最终主分支必须恢复完整的 `drizzle/`，包含连续的 `0000`–`0011`。无论走
`__appgarden_migrations` 的标准路径还是经批准的 bridge 路径，都重新运行：
...
`dist/.openai/drizzle/`。随后保存并部署 owner-only 最终版本。对于已确认
`__appgarden_migrations` 精确包含 `0000`–`0004` 的 Sites，标准迁移器会跳过
这五项，只执行 `0005`–`0011`；不得把后续迁移预先写入任何台账。
